...
class MultimodalRAGPipeline:
    ...
    def __init__(self, config: Dict):
        ...
        # 4. Base de Datos Vectorial (Retriever)
        index_path = os.path.join(rag_conf.get("index_path", ""), rag_conf.get("index_filename", ""))
        meta_path = os.path.join(rag_conf.get("index_path", ""), rag_conf.get("metadata_filename", ""))
        
        if not os.path.exists(index_path) or not os.path.exists(meta_path):
            raise FileNotFoundError(f"❌ No se encuentran los índices en {index_path}")
            
        self.retriever = DamageRAGRetriever(index_path, meta_path)
    ...
    def _aggregate_findings(self, findings: List[Dict], total_crops: int) -> str:
        ...
        damage_hits = [f for f in findings if f.get('has_damage', False)]
        clean_hits = [f for f in findings if not f.get('has_damage', False)]
        
        damage_ratio = len(damage_hits) / len(findings)
        
        # 2. Identificar Zonas y Tipos
        detected_zones = Counter([f.get('zone_description', 'unknown') for f in damage_hits])
        detected_types = Counter([f.get('damage_type', 'unknown') for f in damage_hits])
        ...
        best_matches = sorted(findings, key=lambda x: x.get('distance', 0), reverse=True)[:3]
        
        if best_matches:
            lines.append("\n**Top Reference Cases from Database:**")
            for i, m in enumerate(best_matches, 1):
                status = "Damaged" if m.get('has_damage') else "Clean"
                detail = m.get('damage_type', 'No defects')
                zone = m.get('zone_description', 'Unknown zone')
                conf = m.get('distance', 0) * 100
